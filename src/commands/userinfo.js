...
module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        const embed = new MessageEmbed().setTitle(`${member.user.username}님의 정보`).setFooter(member.user.username, member.user.displayAvatarURL()).setThumbnail(member.user.displayAvatarURL()).setColor(member.displayHexColor === "#000000" ? "#FFFFFF" : member.displayHexColor).setTimestamp()
            .addField(`${bot.emojis.cache.find(x => x.name == ":discord:")} 유저 이름`, `**${member.user.username}**`)
            .addField(`${bot.emojis.cache.find(x => x.name == ":discord:")} 디스플레이 이름`, stripIndents`**${member.displayName}**`)
            .addField(`${bot.emojis.cache.find(x => x.name == ":discord:")} 디스코드 태그`, `**${member.user.tag}**`)
            .addField('🆔 ID', stripIndents`**${member.user.id}**`)

        if (member.user.presence.status !== "offline" && member.user.bot === false) {
            if (member.user.presence.botStatus.desktop) {
                embed.addField(`${bot.emojis.cache.find(x => x.name == "discord")} 디스코드 클라이언트`, `**🖥 디스코드 앱**`)
            } else if (member.user.presence.botStatus.web) {
                embed.addField(`${bot.emojis.cache.find(x => x.name == "discord")} 디스코드 클라이언트`, `**⌨ 웹**`)
            } else if (member.user.presence.botStatus.mobile) {
                embed.addField(`${bot.emojis.cache.find(x => x.name == "discord")} 디스코드 클라이언트`, `**📱 모바일**`)
            }
        }
